...
def openConfig():
    ...
    Returns time of the last fetch or current time if there is no config file.'''

    config = configparser.ConfigParser()
    curtime = str(datetime.utcnow())
    conf = {'server': 0,'login': 0, 'password': 0, 'time': 0}
    if config.read('example.ini'):
        conf['time'] = datetime.strptime(config['Last fetch']['Time'],'%Y-%m-%d %H:%M:%S.%f')
...
def sendEmail():
    ...
    mail.send_message(msg,'rodion.goritskov@gmail.com','rodion@goritskov.com')
